...
def test_backend_manager(tmp_path: Path):
    ...
    group = open_group_wrapper(store=store, mode="a")
    backend = manager.get_backend(backend_name="json", group=group)
    assert isinstance(backend, JsonTableBackend)

    backend = manager.get_backend(group=group)
    assert isinstance(backend, AnnDataBackend)

    with pytest.raises(NgioTablesValueError):
        manager.get_backend(group=group, backend_name="non_existent_backend")
...
def test_json_backend(tmp_path: Path):
    ...
    backend.set_group(group, index_type="str")
    ...
    meta = backend._group.attrs.asdict()
...
def test_anndata_backend(tmp_path: Path):
    ...
    group = zarr.open_group(store=str(store), mode="a", zarr_format=2)
    backend = AnnDataBackend()
    backend.set_group(group, index_type="int")
...
def test_anndata_to_dataframe(index_label: str | None, index_type: str):
    ...
    dataframe = convert_anndata_to_pandas(
        ...
        index_type=index_type,  # type: ignore  # test parametrize yields str, function expects Literal
    )
