...
from ngio_tables._utils import NgioTablesTableValidationError, NgioTablesValueError
...
def _validate_index_key_df(pandas_df: DataFrame, index_key: str | None) -> DataFrame:
    ...
    Raises:
        NgioTablesTableValidationError: If index key is not found in DataFrame.
    """
    ...
    raise NgioTablesTableValidationError(
        f"Index key '{index_key}' is not found in DataFrame"
    )
...
def _validate_cast_index_dtype_df(
    ...
    Raises:
        NgioTablesTableValidationError: If index cannot be cast to the specified type.
        NgioTablesValueError: If index_type is not 'str' or 'int'.
    """
...
def _check_for_mixed_types(series: pd.Series) -> None:
    ...
    Raises:
        NgioTablesTableValidationError: If the column has mixed types.
    """
...
def _check_for_supported_types(series: pd.Series) -> Literal["str", "int", "numeric"]:
    ...
    Raises:
        NgioTablesTableValidationError: If the column has unsupported types.
    """
...
def normalize_anndata(
    ...
    if index_key is None:
        return anndata
    obs = _validate_index_key_df(cast("DataFrame", anndata.obs), index_key)
...
def convert_polars_to_pandas(
    ...
    if isinstance(polars_df, LazyFrame):
        polars_df = cast("PolarsDataFrame", polars_df.collect())
...
def normalize_table(
    ...
    raise NgioTablesValueError(f"Unsupported table type: {type(table_data)}")
