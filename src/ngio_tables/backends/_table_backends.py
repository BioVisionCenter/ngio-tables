...
from ngio_tables.backends._utils import TabularData


class TableBackendProtocol(Protocol):
    """Protocol for table backend implementations."""
    ...
    def get_backend(
        ...
        *,
        ...
        if backend_name not in self._implemented_backends:
            raise NgioTablesValueError(f"Table backend {backend_name} not implemented.")
        backend = self._implemented_backends[backend_name]()
        backend.set_group(group=group, index_key=index_key, index_type=index_type)
        return backend
