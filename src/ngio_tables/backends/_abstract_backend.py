...
class AbstractTableBackend(ABC):
    ...
    def set_group(
        ...
        """Attach a zarr group to the backend.
    ...
    def write_metadata(self, metadata: dict | None = None) -> None:
        ...
        # Reopen to pick up any attrs written
        # by the backend (e.g., anndata encoding-type)
        fresh_group = zarr.open_group(
            store=self._group.store,
            path=self._group.path,
            mode="r+",
        )
        attrs = fresh_group.attrs.asdict()
        ...
        fresh_group.attrs.update(metadata)
        self._group = fresh_group
